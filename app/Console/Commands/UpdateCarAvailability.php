...
class UpdateCarAvailability extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($mobils as $mobil) {
            ...
            // Ambil semua booking untuk mobil ini dengan status pembatalan 'Terverifikasi'
            $bookings = bkuser::where('id_mobil', $mobil->id)
                          ->where('pembatalan', 'Terverifikasi') // Kondisi pertama: pembatalan terverifikasi
                          ->where('status', 'Pending') // Kondisi kedua: status booking pending
                          ->get();
